CREATE OR REPLACE FUNCTION public.create_lpd_baru(p_tgl_buat date, p_tgl_kegiatan date, p_tgl_selesai date, p_jenis_perjadin text, p_id_rangka integer, p_id_tempat integer, p_id_kepala uuid, p_petugas_ids uuid[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  v_no_surat := '090/' || lpad(v_nomor::text, 4, '0') || '/P.KI.' || v_tahun::text;
  v_no_surat_slug := '090_' || lpad(v_nomor::text, 4, '0') || '_P.KI.' || v_tahun::text;
...
ALTER TABLE public.nomor_surat_allocation
  ADD CONSTRAINT alloc_range_4digit CHECK (range_start >= 1 AND range_end <= 9999 AND range_end >= range_start);